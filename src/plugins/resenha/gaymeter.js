...
module.exports = {
  ...
  async execute({ client, from, info, reply, reagir, args, sender }) {
    ...
    try {
      const img = await drawMeter({ title: 'Gaymetro', emoji: '🏳️‍🌈', name: uname, percent: pct })
      ...
      await client.sendMessage(from, { text: `🏳️‍🌈 ${uname} — *${pct}%*`, mentions: [target] }, { quoted: info })
    }
  }
}
